...
EXP_ST u16* trace_bits2;

EXP_ST u16  virgin_bits2[MAP_SIZE],     /* Regions yet untouched by fuzzing */
           virgin_tmout2[MAP_SIZE],    /* Bits we haven't seen in tmouts   */
           virgin_crash2[MAP_SIZE];
...
struct queue_entry {
  ...
  double distance;  //change            /* Distance to targets              */
  int confnumber;
...
static inline u8 has_new_bits(u8* virgin_map) {
  ...
  u64* confNum = (u64*) (trace_bits + MAP_SIZE + 16);
  if (*total_count > 0)
  ...
  u32* confNum = (u32*) (trace_bits + MAP_SIZE + 8);
...
EXP_ST void setup_shm(void) {
  ...
  if (!in_bitmap) memset(virgin_bits2, 255, MAP_SIZE);

  memset(virgin_tmout2, 255, MAP_SIZE);
  memset(virgin_crash2, 255, MAP_SIZE);
  ...
  if (!dumb_mode) setenv(CONFSHM_ENV_VAR, shm_str2, 1);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    //修改开始
    //如果该测试用例是有趣的，则用它更新library.
    stage_name = "add_to_library";
    string strip_sql = g_mutator.extract_struct(g_current_ir); //g_current_ir将会在fuzz_one的abandon_entry阶段删除
    ...
    fn = alloc_printf("%s/queue/id:%06u,orig:new4.txt", out_dir, queued_paths);
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  if(cur_confnumber > 0){
    // u8* fn = alloc_printf("%s/confnumber", out_dir);
    // s32 fd;
    // FILE* f;
    ...
    // f = fdopen(fd, "w");
    // if (!f) PFATAL("fdopen() failed");
    // fprintf(f,"confnumbber == %d\n",cur_confnumber);
    // fclose(f);

    // printf("confnumber:%d\n",cur_confnumber);
     string p;
     int i = UR(100);
     switch(cur_confnumber){
       case 1: 
           p = "PRAGMA analysis_limit = 1000;";
           break;
       case 2:
           if(i < 33) p = "PRAGMA auto_vacuum = 0;";
           else if(i>=33 && i<66) p = "PRAGMA auto_vacuum = 1;";
           else p = "PRAGMA auto_vacuum = 2;";
           break;
       case 3:
           if(i<50) p = "PRAGMA busy_timeout = -1;";
           else p = "PRAGMA busy_timeout = 21474836;";
           break;
       case 4:
           if(i<50) p = "PRAGMA cache_size = 10;";
           else p = "PRAGMA cache_size = 21474836;";
           break;
       case 5:
           p = "PRAGMA case_sensitive_like = 0;";
           break;
       case 6:
           p = "PRAGMA cell_size_check = 1;";
           break;
       case 7:
           p = "PRAGMA count_changes = 1;";
           break;
       case 8:
           p = "PRAGMA defer_foreign_keys = 1;";
           break;
       case 9:
           p = "PRAGMA empty_result_callbacks = 1;";
           break;
       case 10:
           if(i<50) p = "PRAGMA encoding = 'UTF-16';";
           else p = "PRAGMA encoding = 'UTF-16be';";
           break;
       case 11:
           p = "PRAGMA full_column_names = 1;";
           break;
       case 12:
           if(i < 16) p = "PRAGMA journal_mode = DELETE;";
           else if(i>=16 && i<32) p = "PRAGMA journal_mode = TRUNCATE;";
           else if(i>=32 && i<48)  p = "PRAGMA journal_mode = PERSIST;";
           else if(i>=48 && i<64)  p = "PRAGMA journal_mode = MEMORY;";
           else if(i>=64 && i<80)  p = "PRAGMA journal_mode = WAL;";
           else p = "PRAGMA journal_mode = OFF;";
           break;
       case 13:
           if(i<50) p = "PRAGMA journal_size_limit = -100;";
           else p = "PRAGMA journal_size_limit = 21474836;";
           break;
       case 14:
           if(i<50) p = "PRAGMA locking_mode = NORMAL;";
           else p = "PRAGMA locking_mode = EXCLUSIVE;";
           break;
       case 15:
           if(i<50) p = "PRAGMA max_page_count = 0;";
           else p = "PRAGMA max_page_count = 1073741823;";
           break;
       case 16:
           if(i<50) p = "PRAGMA mmap_size = 0;";
           else p = "PRAGMA mmap_size = 2147418112;";
           break;
       case 17:
           if(i<50) p = "PRAGMA page_size = 512;";
           else p = "PRAGMA page_size = 65536;";
           break;
       case 18:
           p = "PRAGMA query_only = 1;";
           break;
       case 19:
           p = "PRAGMA recursive_triggers = 1;";
           break;
       case 20:
           if(i<50) p = "PRAGMA secure_delete = 1;";
           else p = "PRAGMA secure_delete = FAST;";
           break;
       case 21:
           if(i < 33) p = "PRAGMA synchronous = 0;";
           else if(i>=33 && i<66) p = "PRAGMA synchronous = 1;";
           else p = "PRAGMA synchronous = 3;";
           break;
       case 22:
           if(i<50) p = "PRAGMA temp_store = 1;";
           else p = "PRAGMA temp_store = 2;";
           break;
       case 23:
           if(i<50) p = "PRAGMA wal_autocheckpoint = 0;";
           else p = "PRAGMA wal_autocheckpoint = 107374182;";
           break;
       case 24:
           p = "PRAGMA wal_checkpoint;";
           break;
       case 25:
           p = "PRAGMA writable_schema = 1;";
           break;
       case 26:
           p = "PRAGMA cache_spill = 0;";
           break; 
       case 27:
           p = "PRAGMA fullsync = 1;";
           break;
       default: p = "PRAGMA analysis_limit = 0;";
...
//AFL 的calculate_score函数是对 seed 进行打分，打分的分数决定对 seed fuzzing的时间长度
static u32 calculate_score(struct queue_entry* q) {
  ...
  if (q->distance > 0) {
    ...
    if (max_distance != min_distance)
      normalized_d = (q->distance - min_distance) / (max_distance - min_distance);// 首先归一化距离
...
static u8 fuzz_one(char** argv) {
  ...
  if(program_root == NULL){
     ...
     try{
      ...
      int smax = calculate_score(queue_cur)/10;
       //printf("smax:%d\n",smax);
      stage_cur = 0;
...
int main(int argc, char** argv) {
  ...
  SAYF(cCYA "conffuzz (yeah!) " cBRI VERSION cRST "\n");
  ...
  do_libary_initialize();//修改，增添library初始化函数
