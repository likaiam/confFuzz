...
cl::opt<std::string> ConfBBfile(
    "confbb",
    cl::desc("configuration and basic block map file"),
    cl::value_desc("confbb"));
...
bool AFLCoverage::runOnModule(Module &M) {
  ...
  std::map<int,std::string> conf_to_bb;
  ...
  } else if (!DistanceFile.empty() && !ConfBBfile.empty()) {

    std::ifstream cf(DistanceFile);
     std::ifstream bf(ConfBBfile);
    if (cf.is_open() && bf.is_open()) {
      ...
      while (getline(bf, bline)) {
        ...
        std::size_t pos = bline.find(",");
        int confNum =(int)(atoi(bline.substr(0,pos).c_str()));
        std::string bb_name = bline.substr(pos+1, bline.length());

        conf_to_bb.emplace(confNum, bb_name);
  ...
  if (is_aflgo_preprocessing) {
    ...
    ConstantInt *ConfVarLoc = ConstantInt::get(LargestType, MAP_SIZE + 16);
#else
    IntegerType *LargestType = Int32Ty;
    ConstantInt *MapCntLoc = ConstantInt::get(LargestType, MAP_SIZE + 4);
    ConstantInt *ConfVarLoc = ConstantInt::get(LargestType, MAP_SIZE + 8);
#endif
    ...
    GlobalVariable *ConfMapPtr =
        new GlobalVariable(M, PointerType::get(Int16Ty, 0), false,
                           GlobalValue::ExternalLinkage, 0, "__conf_area_ptr");
    ...
    for (auto &F : M) {
      ...
      for (auto &BB : F) {
          ...
          if (!bb_name.empty()) {
            ...
            if (find(basic_blocks.begin(), basic_blocks.end(), bb_name) == basic_blocks.end()) {
              ...
              if (AFL_R(100) < dinst_ratio) {
                ...
                for (its = conf_to_bb.begin(); its != conf_to_bb.end(); ++its)
                  if (its->second.compare(bb_name) == 0)
                    confNum = its->first;
        ...
        if (distance >= 0) {
          ...
          if(confNum>0){
          ConstantInt *Conf =ConstantInt::get(LargestType, (unsigned) confNum);
          Value *ConfVarPtr = IRB.CreateBitCast(IRB.CreateGEP(MapPtr, ConfVarLoc), LargestType->getPointerTo());
          IRB.CreateStore(Conf, ConfVarPtr)->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
          unsigned int cur_loc2 = AFL_R(MAP_SIZE/2);
          ConstantInt *CurLoc2 = ConstantInt::get(Int16Ty, cur_loc2);
          LoadInst *ConfPtr = IRB.CreateLoad(ConfMapPtr);
          ConfPtr->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
          Value *ConfPtrIdx =IRB.CreateBitCast(IRB.CreateGEP(ConfPtr, CurLoc2),Int16Ty->getPointerTo());
          LoadInst *Counter2 = IRB.CreateLoad(ConfPtrIdx);
          Counter2->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
          Value *Incr2 = IRB.CreateAdd(Counter2, ConstantInt::get(Int16Ty,1));
          IRB.CreateStore(Incr2, ConfPtrIdx)
              ->setMetadata(M.getMDKindID("nosanitize"), MDNode::get(C, None));
