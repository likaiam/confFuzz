...
u8  __conf_area_initial[MAP_SIZE];
u8* __conf_area_ptr = __conf_area_initial;
...
static void __afl_map_shm(void) {
  ...
  u8 *id_conf = getenv(CONFSHM_ENV_VAR);
  ...
  if (id_conf) {
    ...
    u32 shm_id = atoi(id_conf);

    __conf_area_ptr = shmat(shm_id, NULL, 0);

    /* Whooooops. */

    if (__conf_area_ptr == (void *)-1) _exit(1);
